...
int main(int argc, char *argv[]) {
  ...
  if(argc < 4) {
    fprintf(stderr, "Usage: gremlin <BAM> <reference FASTA> <name map>\n");
    fprintf(stderr, "  <name map>: comma-separated text where each line is 'seq_name\tgenome_name'\n");
  ...
  while ((ret_val = sam_read1(bam, header, aln)) >= 0) {
    ...
    if(aln->core.n_cigar > 1 || bam_cigar_opchr(cigar[0]) != 'M') { // not perfect
      continue;
    }
    int window = 500;
    for(i = (pos - window > 0 ? pos - window : 0); i < (pos + window < rlen_array[tid] ? pos + window : rlen_array[tid] - 1); i++) {
      covg[tid][i]++;
    }
    /*
  ...
  fprintf(stdout, "reference_name\tgenome_size\taligned_bp\tcovered_loci\tcovered_pct\tavg_coverage\tcoverage_of_covered_loci\texpected_covered\tcovered/expected\n");
  char* name;
  for (i = 0; i < header->n_targets; i++) {
  ...
  for (bin = 0; bin < kh_end(n2s); ++bin) {
    if (kh_exist(n2s, bin)) {
      ...
      if(rs.total_coverage > 0) {
        uint32_t expect_covered = (uint32_t)round(rs.total_loci - pow(M_E, rs.total_coverage * log(rs.total_loci - 1) - (rs.total_coverage - 1) * log(rs.total_loci)));
        fprintf(stdout, "%s\t%u\t%u\t%u\t%f\t%f\t%f\t%u\t%f\n", kh_key(n2s, bin), rs.total_loci, rs.total_coverage, rs.covered_loci, (float)rs.covered_loci/rs.total_loci, (float)rs.total_coverage/rs.total_loci, (float)rs.total_coverage/rs.covered_loci, expect_covered, (float)rs.covered_loci/expect_covered);
      }
    }
  }
